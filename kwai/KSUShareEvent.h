//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "GPBMessage.h"

@class NSString;

@interface KSUShareEvent : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(copy, nonatomic) NSString *URL; // @dynamic URL;
@property(copy, nonatomic) NSString *authorId; // @dynamic authorId;
@property(nonatomic) int contentType; // @dynamic contentType;
@property(copy, nonatomic) NSString *expTag; // @dynamic expTag;
@property(copy, nonatomic) NSString *expTag0; // @dynamic expTag0;
@property(copy, nonatomic) NSString *failureReason; // @dynamic failureReason;
@property(copy, nonatomic) NSString *photoId; // @dynamic photoId;
@property(copy, nonatomic) NSString *photoInfo; // @dynamic photoInfo;
@property(nonatomic) int platform; // @dynamic platform;
@property(nonatomic) int source; // @dynamic source;
@property(nonatomic) int status; // @dynamic status;

@end

