//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "KSUPageResponse.h"

@class NSDictionary, NSNumber;

@interface KSFeedCommentsResponse : KSUPageResponse
{
    NSNumber *_commentCount;
    NSDictionary *_subCommentsMap;
}

+ (id)mj_replacedKeyFromPropertyName;
+ (id)mj_objectClassInArray;
@property(retain, nonatomic) NSDictionary *subCommentsMap; // @synthesize subCommentsMap=_subCommentsMap;
@property(retain, nonatomic) NSNumber *commentCount; // @synthesize commentCount=_commentCount;
- (void).cxx_destruct;

@end

