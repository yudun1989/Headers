//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "WBOpenUrlContextBase.h"

@class NSDictionary, NSString;

@interface WBOpenUrlGetTaobaoCouponContext : WBOpenUrlContextBase
{
    NSString *_supplierid;
    NSString *_uuid;
    NSString *_couponinstancesource;
    NSDictionary *_ybhpss;
    NSString *_asac;
    NSString *_owner_uid;
    NSString *_successtips;
    NSString *_failtips;
}

@property(retain, nonatomic) NSString *failtips; // @synthesize failtips=_failtips;
@property(retain, nonatomic) NSString *successtips; // @synthesize successtips=_successtips;
@property(retain, nonatomic) NSString *owner_uid; // @synthesize owner_uid=_owner_uid;
@property(retain, nonatomic) NSString *asac; // @synthesize asac=_asac;
@property(retain, nonatomic) NSDictionary *ybhpss; // @synthesize ybhpss=_ybhpss;
@property(retain, nonatomic) NSString *couponinstancesource; // @synthesize couponinstancesource=_couponinstancesource;
@property(retain, nonatomic) NSString *uuid; // @synthesize uuid=_uuid;
@property(retain, nonatomic) NSString *supplierid; // @synthesize supplierid=_supplierid;
- (void).cxx_destruct;
- (void)dealloc;
- (void)startContext;
- (void)parseUrlAndRefreshValues:(id)arg1;

@end

