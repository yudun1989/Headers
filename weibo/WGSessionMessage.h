//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

@class WGChatMessageInfo;

@interface WGSessionMessage : NSObject
{
    _Bool _showDateLabel;
    WGChatMessageInfo *_chatInfo;
}

@property(nonatomic) _Bool showDateLabel; // @synthesize showDateLabel=_showDateLabel;
@property(retain, nonatomic) WGChatMessageInfo *chatInfo; // @synthesize chatInfo=_chatInfo;
- (void).cxx_destruct;
- (void)minuteOffSetStart:(id)arg1 end:(id)arg2;
- (id)changeTheDateString:(id)arg1;

@end

