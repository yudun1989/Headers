//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "AFJSONResponseSerializer.h"

#import "TTJSONResponseSerializerProtocol-Protocol.h"

@class NSSet, NSString;

@interface HTSAccountJSONResponseSerializer : AFJSONResponseSerializer <TTJSONResponseSerializerProtocol>
{
}

+ (id)handleError:(id)arg1 responseResult:(id)arg2 exceptionInfo:(id *)arg3 treatExceptionAsError:(_Bool)arg4;
+ (id)dictionaryWithJSONData:(id)arg1 error:(id *)arg2;
+ (id)serializer;
- (id)resultByResponseDictionary:(id)arg1 error:(id *)arg2;
- (id)responseObjectForResponse:(id)arg1 jsonObj:(id)arg2 responseError:(id)arg3 resultError:(id *)arg4;
- (id)init;

// Remaining properties
@property(copy, nonatomic) NSSet *acceptableContentTypes;
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

