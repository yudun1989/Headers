//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIViewController.h>

#import "BTDRouterViewControllerProtocol-Protocol.h"
#import "UITextFieldDelegate-Protocol.h"

@class AWEAccountPhoneNumberViewController, AWECaptchaInputView, AWELoginButton, AWELoginGradientView, NSNumber, NSString, NSTimer, UIButton, UILabel;

@interface AWECaptchaViewController : UIViewController <UITextFieldDelegate, BTDRouterViewControllerProtocol>
{
    _Bool _isCounting;
    NSString *_password;
    NSString *_enterFrom;
    UILabel *_titleLabel;
    UILabel *_hintLabel;
    AWECaptchaInputView *_captchaInputView;
    AWELoginButton *_nextButton;
    UIButton *_backButton;
    AWELoginGradientView *_backgroundView;
    UIButton *_stateButton;
    unsigned long long _currentCounting;
    NSTimer *_timer;
    NSString *_phoneNumber;
    NSNumber *_retryTime;
    unsigned long long _state;
    unsigned long long _previousState;
    NSString *_captchaValue;
    unsigned long long _useType;
    double _nextButtonBottom;
    AWEAccountPhoneNumberViewController *_rebindPhoneNumberViewController;
}

+ (void)load;
@property(retain, nonatomic) AWEAccountPhoneNumberViewController *rebindPhoneNumberViewController; // @synthesize rebindPhoneNumberViewController=_rebindPhoneNumberViewController;
@property(nonatomic) double nextButtonBottom; // @synthesize nextButtonBottom=_nextButtonBottom;
@property(nonatomic) unsigned long long useType; // @synthesize useType=_useType;
@property(retain, nonatomic) NSString *captchaValue; // @synthesize captchaValue=_captchaValue;
@property(nonatomic) unsigned long long previousState; // @synthesize previousState=_previousState;
@property(nonatomic) unsigned long long state; // @synthesize state=_state;
@property(retain, nonatomic) NSNumber *retryTime; // @synthesize retryTime=_retryTime;
@property(copy, nonatomic) NSString *phoneNumber; // @synthesize phoneNumber=_phoneNumber;
@property(retain, nonatomic) NSTimer *timer; // @synthesize timer=_timer;
@property(nonatomic) unsigned long long currentCounting; // @synthesize currentCounting=_currentCounting;
@property(retain, nonatomic) UIButton *stateButton; // @synthesize stateButton=_stateButton;
@property(retain, nonatomic) AWELoginGradientView *backgroundView; // @synthesize backgroundView=_backgroundView;
@property(retain, nonatomic) UIButton *backButton; // @synthesize backButton=_backButton;
@property(retain, nonatomic) AWELoginButton *nextButton; // @synthesize nextButton=_nextButton;
@property(retain, nonatomic) AWECaptchaInputView *captchaInputView; // @synthesize captchaInputView=_captchaInputView;
@property(retain, nonatomic) UILabel *hintLabel; // @synthesize hintLabel=_hintLabel;
@property(retain, nonatomic) UILabel *titleLabel; // @synthesize titleLabel=_titleLabel;
@property(copy, nonatomic) NSString *enterFrom; // @synthesize enterFrom=_enterFrom;
@property(copy, nonatomic) NSString *password; // @synthesize password=_password;
@property(nonatomic) _Bool isCounting; // @synthesize isCounting=_isCounting;
- (void).cxx_destruct;
- (void)keyboardWillHideNotificaiton:(id)arg1;
- (void)keyboardWillShowNotification:(id)arg1;
- (void)_trackToastError;
- (void)backAction;
- (void)stateButtonAction;
- (void)_handleError:(id)arg1 api:(id)arg2;
- (void)nextAction;
- (void)textFieldDidEndEditing:(id)arg1;
- (_Bool)textField:(id)arg1 shouldChangeCharactersInRange:(struct _NSRange)arg2 replacementString:(id)arg3;
- (void)_dismiss;
- (void)_stopLoading;
- (void)_startLoading;
- (void)_updateNextButtonState;
- (void)_setStateButtonToPreviousState;
- (void)_resetStateButton;
- (void)_getCaptcha:(_Bool)arg1;
- (void)_countingDown;
- (void)_stopCounting;
- (void)_startCounting;
- (void)_updateState;
- (void)_adaptUI;
- (void)_updateHintLabel;
- (void)_configureUseType;
- (void)viewDidDisappear:(_Bool)arg1;
- (void)viewWillDisappear:(_Bool)arg1;
- (void)viewDidAppear:(_Bool)arg1;
- (void)viewDidLoad;
- (_Bool)configWithRouterParamDict:(id)arg1;
- (id)initWithType:(unsigned long long)arg1 phoneNumber:(id)arg2;
- (void)dealloc;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

