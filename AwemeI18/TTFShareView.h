//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIView.h>

#import "CAAnimationDelegate-Protocol.h"

@class NSMutableArray, NSString, TTFShareViewModel, TTFantasyButton, TTFantasyLabel;

@interface TTFShareView : UIView <CAAnimationDelegate>
{
    _Bool _reliveTip;
    _Bool _isShareViewShow;
    TTFShareViewModel *_viewModel;
    UIView *_backgroundMaskView;
    UIView *_containerView;
    TTFantasyButton *_closeButton;
    TTFantasyLabel *_titleLabel;
    NSMutableArray *_paragraphLabels;
    TTFantasyLabel *_inviteCodeValueLabel;
    TTFantasyLabel *_inviteCodeNameLabel;
    NSMutableArray *_shareButtons;
}

+ (void)linearAnimations:(CDUnknownBlockType)arg1 completion:(CDUnknownBlockType)arg2;
@property(nonatomic) _Bool isShareViewShow; // @synthesize isShareViewShow=_isShareViewShow;
@property(retain, nonatomic) NSMutableArray *shareButtons; // @synthesize shareButtons=_shareButtons;
@property(retain, nonatomic) TTFantasyLabel *inviteCodeNameLabel; // @synthesize inviteCodeNameLabel=_inviteCodeNameLabel;
@property(retain, nonatomic) TTFantasyLabel *inviteCodeValueLabel; // @synthesize inviteCodeValueLabel=_inviteCodeValueLabel;
@property(retain, nonatomic) NSMutableArray *paragraphLabels; // @synthesize paragraphLabels=_paragraphLabels;
@property(retain, nonatomic) TTFantasyLabel *titleLabel; // @synthesize titleLabel=_titleLabel;
@property(retain, nonatomic) TTFantasyButton *closeButton; // @synthesize closeButton=_closeButton;
@property(retain, nonatomic) UIView *containerView; // @synthesize containerView=_containerView;
@property(retain, nonatomic) UIView *backgroundMaskView; // @synthesize backgroundMaskView=_backgroundMaskView;
@property(retain, nonatomic) TTFShareViewModel *viewModel; // @synthesize viewModel=_viewModel;
@property(nonatomic) _Bool reliveTip; // @synthesize reliveTip=_reliveTip;
- (void).cxx_destruct;
- (id)paragraphColors;
- (void)configDefaultAnimation:(id)arg1 fromValue:(id)arg2 toValue:(id)arg3;
- (void)animationDidStop:(id)arg1 finished:(_Bool)arg2;
- (void)longPress:(id)arg1;
- (void)hide;
- (void)showInView:(id)arg1 shareCompletion:(CDUnknownBlockType)arg2;
- (id)getTitleText;
- (void)updateData;
- (void)shareButtonTouch:(id)arg1;
- (void)layoutSubviews;
- (void)initComponents;
- (id)initWithFrame:(struct CGRect)arg1 reliveTip:(_Bool)arg2;
- (id)initWithFrame:(struct CGRect)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

