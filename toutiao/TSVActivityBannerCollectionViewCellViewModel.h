//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

@class TSVActivityBannerModel, TTImageInfosModel;

@interface TSVActivityBannerCollectionViewCellViewModel : NSObject
{
    TTImageInfosModel *_coverImageModel;
    TSVActivityBannerModel *_model;
}

@property(retain, nonatomic) TSVActivityBannerModel *model; // @synthesize model=_model;
@property(retain, nonatomic) TTImageInfosModel *coverImageModel; // @synthesize coverImageModel=_coverImageModel;
- (void).cxx_destruct;
- (void)bindWithModel;
- (id)initWithModel:(id)arg1;

@end

