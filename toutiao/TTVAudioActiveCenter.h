//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

#import "TTVPlayerContext-Protocol.h"

@class NSString, TTVPlayerStateStore;

@interface TTVAudioActiveCenter : NSObject <TTVPlayerContext>
{
    _Bool _isPlayingWhenEnterBackground;
    TTVPlayerStateStore *_playerStateStore;
}

+ (void)setupAudioSessionIsMuted:(_Bool)arg1;
+ (void)beactive;
@property(nonatomic) _Bool isPlayingWhenEnterBackground; // @synthesize isPlayingWhenEnterBackground=_isPlayingWhenEnterBackground;
@property(retain, nonatomic) TTVPlayerStateStore *playerStateStore; // @synthesize playerStateStore=_playerStateStore;
- (void).cxx_destruct;
- (void)actionChangeCallbackWithAction:(id)arg1 state:(id)arg2;
- (void)ttv_kvo;
- (void)beactive;
- (void)deactive;
- (void)dealloc;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

