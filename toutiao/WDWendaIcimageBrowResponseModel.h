//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "TTResponseModel.h"

@class NSArray, NSNumber, NSString;
@protocol Optional, WDIcImageStructModel><Optional;

@interface WDWendaIcimageBrowResponseModel : TTResponseModel
{
    NSNumber<Optional> *_err_no;
    NSString<Optional> *_err_tips;
    NSArray<WDIcImageStructModel><Optional> *_img_list;
    NSNumber<Optional> *_offset;
    NSNumber<Optional> *_req_count;
    NSNumber<Optional> *_has_more;
    NSString<Optional> *_term;
}

@property(retain, nonatomic) NSString<Optional> *term; // @synthesize term=_term;
@property(retain, nonatomic) NSNumber<Optional> *has_more; // @synthesize has_more=_has_more;
@property(retain, nonatomic) NSNumber<Optional> *req_count; // @synthesize req_count=_req_count;
@property(retain, nonatomic) NSNumber<Optional> *offset; // @synthesize offset=_offset;
@property(copy, nonatomic) NSArray<WDIcImageStructModel><Optional> *img_list; // @synthesize img_list=_img_list;
@property(retain, nonatomic) NSString<Optional> *err_tips; // @synthesize err_tips=_err_tips;
@property(retain, nonatomic) NSNumber<Optional> *err_no; // @synthesize err_no=_err_no;
- (void).cxx_destruct;
- (void)reset;
- (id)init;

@end

