//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "TTVAdActionButtonCommand.h"

#import "TTVAdActionButtonCommandProtocol-Protocol.h"

@class NSString, TTVFeedCellAction, TTVFeedItem, TTVOpenAppParameter;

@interface TTVAdActionTypeAppButtonCommand : TTVAdActionButtonCommand <TTVAdActionButtonCommandProtocol>
{
    TTVOpenAppParameter *_openAppParameter;
}

@property(retain, nonatomic) TTVOpenAppParameter *openAppParameter; // @synthesize openAppParameter=_openAppParameter;
- (void).cxx_destruct;
- (void)playerControlLogoTappedAction;
- (void)playerControlFinishAdAction;
- (void)executeAction;
- (_Bool)openApp;

// Remaining properties
@property(copy, nonatomic) NSString *categoryId;
@property(retain, nonatomic) TTVFeedCellAction *cellAction;
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(retain, nonatomic) TTVFeedItem *feedItem;
@property(readonly) unsigned long long hash;
@property(nonatomic) unsigned long long refer;
@property(nonatomic) _Bool showAlert;
@property(readonly) Class superclass;

@end

