//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "TTResponseModel.h"

@class NSNumber, NSString, WDAnswerStructModel, WDPostAnswerTipsStructModel;
@protocol Optional;

@interface WDWendaCommitPostanswerResponseModel : TTResponseModel
{
    NSNumber<Optional> *_err_no;
    NSString<Optional> *_err_tips;
    NSString<Optional> *_qid;
    NSString<Optional> *_ansid;
    NSString<Optional> *_schema;
    WDPostAnswerTipsStructModel<Optional> *_tips;
    WDAnswerStructModel<Optional> *_answer;
}

@property(retain, nonatomic) WDAnswerStructModel<Optional> *answer; // @synthesize answer=_answer;
@property(retain, nonatomic) WDPostAnswerTipsStructModel<Optional> *tips; // @synthesize tips=_tips;
@property(retain, nonatomic) NSString<Optional> *schema; // @synthesize schema=_schema;
@property(retain, nonatomic) NSString<Optional> *ansid; // @synthesize ansid=_ansid;
@property(retain, nonatomic) NSString<Optional> *qid; // @synthesize qid=_qid;
@property(retain, nonatomic) NSString<Optional> *err_tips; // @synthesize err_tips=_err_tips;
@property(retain, nonatomic) NSNumber<Optional> *err_no; // @synthesize err_no=_err_no;
- (void).cxx_destruct;
- (void)reset;
- (id)init;

@end

