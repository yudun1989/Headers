//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <QQMainProject/QQStoryFeedHeaderInfoView.h>

@class QIMUserIconView, QQAvatarView;

@interface QQStoryFeedUserHeaderInfoView : QQStoryFeedHeaderInfoView
{
    QQAvatarView *_avatarView;
    QIMUserIconView *_qimUserIcon;
}

@property(retain, nonatomic) QIMUserIconView *qimUserIcon; // @synthesize qimUserIcon=_qimUserIcon;
@property(retain, nonatomic) QQAvatarView *avatarView; // @synthesize avatarView=_avatarView;
- (void)onUserRemarkModifyNotification:(id)arg1;
- (void)reloadData;
- (void)setupQIMUserIcon;
- (void)setupNotification;
- (void)setup;
- (void)dealloc;

@end

