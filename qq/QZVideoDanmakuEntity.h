//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <QQMainProject/QZDanmakuEntity.h>

@class NSString, QzoneFeedComment;

@interface QZVideoDanmakuEntity : QZDanmakuEntity
{
    int _lineCount;
    long long _uin;
    NSString *_content;
    QzoneFeedComment *_comment;
    double _appearTime;
    double _hoverTime;
    double _disappearTime;
}

@property(nonatomic) double disappearTime; // @synthesize disappearTime=_disappearTime;
@property(nonatomic) double hoverTime; // @synthesize hoverTime=_hoverTime;
@property(nonatomic) double appearTime; // @synthesize appearTime=_appearTime;
@property(retain, nonatomic) QzoneFeedComment *comment; // @synthesize comment=_comment;
@property(nonatomic) int lineCount; // @synthesize lineCount=_lineCount;
@property(copy, nonatomic) NSString *content; // @synthesize content=_content;
@property(nonatomic) long long uin; // @synthesize uin=_uin;
- (void).cxx_destruct;

@end

