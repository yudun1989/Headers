//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

@class ImmersionPayService, MQPCallback, MQPTrackModel, NSDate, NSDictionary, NSMutableDictionary, NSString;

@interface MQPPaySession : NSObject
{
    _Bool _needUacNextReq;
    _Bool _isNoLoadingView;
    _Bool _isFullPay2ImmPay;
    _Bool _isNextViewL2R;
    _Bool _isFrom3rdApp;
    _Bool _isFirstRequest;
    _Bool _isServerAskLogin;
    NSString *_orderStr;
    NSDictionary *_orderDic;
    NSDictionary *_bizContext;
    NSMutableDictionary *_payContext;
    MQPCallback *_callback;
    ImmersionPayService *_immPayService;
    MQPTrackModel *_log;
    NSString *_sessionId;
    NSString *_bizType;
    NSString *_mspParams;
    NSString *_cookie;
    NSString *_traceId;
    NSString *_wifiName;
    NSString *_netType;
    NSString *_synch;
    NSString *_trade_no;
    NSDate *_openCashierDate;
    NSString *_openCashierTime;
    unsigned long long _checkLoginMode;
    NSString *_currView;
}

+ (id)orderDicFromOrderStr:(id)arg1;
+ (id)sessionWithOrder:(id)arg1;
@property(nonatomic) _Bool isServerAskLogin; // @synthesize isServerAskLogin=_isServerAskLogin;
@property(copy, nonatomic) NSString *currView; // @synthesize currView=_currView;
@property(nonatomic) _Bool isFirstRequest; // @synthesize isFirstRequest=_isFirstRequest;
@property(nonatomic) unsigned long long checkLoginMode; // @synthesize checkLoginMode=_checkLoginMode;
@property(copy, nonatomic) NSString *openCashierTime; // @synthesize openCashierTime=_openCashierTime;
@property(retain, nonatomic) NSDate *openCashierDate; // @synthesize openCashierDate=_openCashierDate;
@property(copy, nonatomic) NSString *trade_no; // @synthesize trade_no=_trade_no;
@property(copy, nonatomic) NSString *synch; // @synthesize synch=_synch;
@property(copy, nonatomic) NSString *netType; // @synthesize netType=_netType;
@property(copy, nonatomic) NSString *wifiName; // @synthesize wifiName=_wifiName;
@property(nonatomic) _Bool isFrom3rdApp; // @synthesize isFrom3rdApp=_isFrom3rdApp;
@property(nonatomic) _Bool isNextViewL2R; // @synthesize isNextViewL2R=_isNextViewL2R;
@property(nonatomic) _Bool isFullPay2ImmPay; // @synthesize isFullPay2ImmPay=_isFullPay2ImmPay;
@property(nonatomic) _Bool isNoLoadingView; // @synthesize isNoLoadingView=_isNoLoadingView;
@property(nonatomic) _Bool needUacNextReq; // @synthesize needUacNextReq=_needUacNextReq;
@property(copy, nonatomic) NSString *traceId; // @synthesize traceId=_traceId;
@property(copy, nonatomic) NSString *cookie; // @synthesize cookie=_cookie;
@property(copy, nonatomic) NSString *mspParams; // @synthesize mspParams=_mspParams;
@property(copy, nonatomic) NSString *bizType; // @synthesize bizType=_bizType;
@property(copy, nonatomic) NSString *sessionId; // @synthesize sessionId=_sessionId;
@property(retain, nonatomic) MQPTrackModel *log; // @synthesize log=_log;
@property(nonatomic) __weak ImmersionPayService *immPayService; // @synthesize immPayService=_immPayService;
@property(retain, nonatomic) MQPCallback *callback; // @synthesize callback=_callback;
@property(retain, nonatomic) NSMutableDictionary *payContext; // @synthesize payContext=_payContext;
@property(retain, nonatomic) NSDictionary *bizContext; // @synthesize bizContext=_bizContext;
@property(retain, nonatomic) NSDictionary *orderDic; // @synthesize orderDic=_orderDic;
@property(copy, nonatomic) NSString *orderStr; // @synthesize orderStr=_orderStr;
- (void).cxx_destruct;
- (void)stopTrack;
- (void)startTrack;
- (id)backSchemeString;
- (id)fetchMspParams;
@property(readonly, nonatomic) _Bool isBizPayIntegration;
- (void)setObject:(id)arg1 forKeyedSubscript:(id)arg2;
- (id)objectForKeyedSubscript:(id)arg1;
- (void)dealloc;
- (id)init;

@end

