//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

@class NSString;

@interface TBMPBFFOptions : NSObject
{
    _Bool _showHudView;
    _Bool _shouldAutoplay;
    _Bool _loop;
    _Bool _useOptimizedHEVC;
    _Bool _videotoolboxHEVC;
    _Bool _vtbHandleResolutionChange;
    _Bool _isUseProxy;
    _Bool _playerShouldUpthrowImage;
    int _playScenario;
    int _renderType;
    int _playerRenderOrientation;
    int _maxnormalplay;
    int _minnormalplay;
    int _maxfastplay;
    id _ijkFFOptions;
    NSString *_cdnIP;
    NSString *_businessID;
    NSString *_subBusinessType;
    NSString *_userID;
    NSString *_feedID;
    NSString *_anchorAccountID;
    NSString *_videoDefinition;
    NSString *_logToken;
    NSString *_mediaSourceType;
    NSString *_spm;
}

+ (id)defaultPlayerOptions;
@property(nonatomic) int maxfastplay; // @synthesize maxfastplay=_maxfastplay;
@property(nonatomic) int minnormalplay; // @synthesize minnormalplay=_minnormalplay;
@property(nonatomic) int maxnormalplay; // @synthesize maxnormalplay=_maxnormalplay;
@property(nonatomic) int playerRenderOrientation; // @synthesize playerRenderOrientation=_playerRenderOrientation;
@property(nonatomic) int renderType; // @synthesize renderType=_renderType;
@property(nonatomic) _Bool playerShouldUpthrowImage; // @synthesize playerShouldUpthrowImage=_playerShouldUpthrowImage;
@property(nonatomic) _Bool isUseProxy; // @synthesize isUseProxy=_isUseProxy;
@property(copy, nonatomic) NSString *spm; // @synthesize spm=_spm;
@property(copy, nonatomic) NSString *mediaSourceType; // @synthesize mediaSourceType=_mediaSourceType;
@property(copy, nonatomic) NSString *logToken; // @synthesize logToken=_logToken;
@property(copy, nonatomic) NSString *videoDefinition; // @synthesize videoDefinition=_videoDefinition;
@property(copy, nonatomic) NSString *anchorAccountID; // @synthesize anchorAccountID=_anchorAccountID;
@property(copy, nonatomic) NSString *feedID; // @synthesize feedID=_feedID;
@property(copy, nonatomic) NSString *userID; // @synthesize userID=_userID;
@property(copy, nonatomic) NSString *subBusinessType; // @synthesize subBusinessType=_subBusinessType;
@property(copy, nonatomic) NSString *businessID; // @synthesize businessID=_businessID;
@property(nonatomic) _Bool vtbHandleResolutionChange; // @synthesize vtbHandleResolutionChange=_vtbHandleResolutionChange;
@property(nonatomic) _Bool videotoolboxHEVC; // @synthesize videotoolboxHEVC=_videotoolboxHEVC;
@property(nonatomic) _Bool useOptimizedHEVC; // @synthesize useOptimizedHEVC=_useOptimizedHEVC;
@property(copy, nonatomic) NSString *cdnIP; // @synthesize cdnIP=_cdnIP;
@property(nonatomic) _Bool loop; // @synthesize loop=_loop;
@property(nonatomic) _Bool shouldAutoplay; // @synthesize shouldAutoplay=_shouldAutoplay;
@property(nonatomic) int playScenario; // @synthesize playScenario=_playScenario;
@property(nonatomic) _Bool showHudView; // @synthesize showHudView=_showHudView;
@property(retain, nonatomic) id ijkFFOptions; // @synthesize ijkFFOptions=_ijkFFOptions;
- (void).cxx_destruct;
- (id)getAllOptions;
- (void)setPlayScenarioOptionIntValue:(long long)arg1 forKey:(id)arg2;
- (void)setPlayerOptionIntValue:(long long)arg1 forKey:(id)arg2;
- (void)setSwsOptionIntValue:(long long)arg1 forKey:(id)arg2;
- (void)setCodecOptionIntValue:(long long)arg1 forKey:(id)arg2;
- (void)setFormatOptionIntValue:(long long)arg1 forKey:(id)arg2;
- (void)setPlayerOptionValue:(id)arg1 forKey:(id)arg2;
- (void)setSwsOptionValue:(id)arg1 forKey:(id)arg2;
- (void)setCodecOptionValue:(id)arg1 forKey:(id)arg2;
- (void)setFormatOptionValue:(id)arg1 forKey:(id)arg2;
- (id)init;

@end

