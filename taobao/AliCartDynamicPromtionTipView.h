//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIView.h>

@class NSMutableArray, StyleKitContext;

@interface AliCartDynamicPromtionTipView : UIView
{
    StyleKitContext *_styleKitContext;
    NSMutableArray *_promotionTipArray;
    CDUnknownBlockType _nextClickBlock;
}

@property(copy, nonatomic) CDUnknownBlockType nextClickBlock; // @synthesize nextClickBlock=_nextClickBlock;
@property(retain, nonatomic) NSMutableArray *promotionTipArray; // @synthesize promotionTipArray=_promotionTipArray;
@property(retain, nonatomic) StyleKitContext *styleKitContext; // @synthesize styleKitContext=_styleKitContext;
- (void).cxx_destruct;
- (void)bindData:(id)arg1 isChecking:(_Bool)arg2;
- (void)layoutSubviews;
- (id)initWithFrame:(struct CGRect)arg1;

@end

